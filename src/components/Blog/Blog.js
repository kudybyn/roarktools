...
export default function Blog(){
    ...
    switch(selectFilter){
        case "new":
            filteredData.sort((a, b) => {
                let dateA = new Date(a.createdBy.seconds * 1000 + a.createdBy.nanoseconds / 1000000);
                let dateB = new Date(b.createdBy.seconds * 1000 + b.createdBy.nanoseconds / 1000000);
    ...
    return(
        ...
        <div className='w-full min-h-[100vh] pt-[100px] '>
            <PageHeader title={t('blogs.title')} srcImage={"https://firebasestorage.googleapis.com/v0/b/roarktools-3c762.appspot.com/o/images%2Fblog.jpg?alt=media&token=d52b1882-a970-4f38-93fa-25ea25d85de8"} srcLogo={Blogs}/>
            ...
            <div className="flex items-start flex-col md:flex-row justify-between md:items-center gap-8  w-full">
                            ...
                            <div className="relative">
                                <button onClick={changeFilterOpen} className="flex items-center gap-4 py-2 px-3 border-2 border-black rounded-xl">
                                <div className="text-[21px] font-normal"><strong>{t('blogs.filterBy')} </strong>
                                <span>{t(`blogs.${selectFilter}`)}</span></div>
                                    <img src={ArrowDown} className={`w-[20px] h-[20px] ${openFilter ? "rotate-180" : ""} transition duration-300`} alt="open filter"/>
                                </button>
                                {openFilter && (
                                        <div className="absolute w-full bg-white shadow-lg rounded-xl flex flex-col">
                                            {
                                                filterTypes.filter(item=>item!==selectFilter).map((type,index)=>(
                                                    <button onClick={() => onChangeFilter(type)} className={`border-2 cursor-pointer transition duration-300 hover:bg-[#e0e0e0] px-2 py-3
